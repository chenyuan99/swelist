---
name: Job Application Manager — Gmail, Notion & OfferPlus Sync
description: Syncs job application emails from Gmail and updates statuses in your Notion, OfferPlus, or SQLite tracker — detects offers, rejections, and interview invitations
summary: >
...
metadata:
  openclaw:
    ...
    config:
      - key: tracker_backend
        description: Storage backend — notion, offerplus, or sqlite
        default: notion
      - key: offerplus_token
        description: OfferPlus Agent API token (starts with op_) — from Profile → Agent API Keys
        default: ""
      - key: offerplus_base_url
        description: OfferPlus Agent API base URL
        default: https://lwexhbimtxpndhsidogl.supabase.co/functions/v1/agent-api
      - key: sqlite_db_path
...
- Sync applications into OfferPlus ("sync to offerplus", "update offerplus from gmail", "sync my job status")
...
_(skip if tracker_backend is not offerplus)_

| Field | Value |
|---|---|
| Base URL | `https://lwexhbimtxpndhsidogl.supabase.co/functions/v1/agent-api` |
| Token | User's `op_...` key — from Profile → Agent API Keys |

**Status values** (OfferPlus-specific):
...
**Stage order (never downgrade):** `applied < oa < interview < offer < accepted`
Never overwrite `rejected` with `applied`. Only update `status` and `notes` — never touch `user_id`, `company_link`, or `job_link`.

**Email signal → OfferPlus status mapping:**

| Email signal | OfferPlus status |
|---|---|
| Interview invite / calendar / "schedule time" | `interview` |
| OA / coding challenge / assessment link | `oa` |
| "not moving forward" / "not selected" / "unfortunately" | `rejected` |
| Offer letter / compensation package | `offer` |
| Start date confirmed / accepted | `accepted` |

Skip application confirmation emails ("thank you for applying", "we received your application") — these don't represent a new stage.

---

## Config: Notion Database

_(skip if tracker_backend is sqlite or offerplus)_
...
_(used when tracker_backend is `offerplus` or when syncing to OfferPlus in addition to Notion)_

```
STEP OP0  Resolve token
  TOKEN = profile.offerplus_token
  IF blank: ask user — "Please paste your OfferPlus API token (from Profile → Agent API Keys)."
  BASE  = "https://lwexhbimtxpndhsidogl.supabase.co/functions/v1/agent-api"

STEP OP1  Fetch current applications
  GET $BASE/applications?limit=300
  Build lookup: company_name.toLowerCase() → {id, status}
  Also index individual words (>3 chars) from each company name for fuzzy matching.

STEP OP2  Search Gmail for status signals (run in parallel, newer_than:7d)
  interviews : (interview OR "phone screen" OR "schedule time" OR "next steps") newer_than:7d -in:sent
  rejections : ("not moving forward" OR "decided to move forward with other" OR "unfortunately" OR "not selected" OR "chosen to pursue") newer_than:7d -in:sent
  oas        : ("online assessment" OR "coding challenge" OR hackerrank OR codesignal OR codility OR "take-home assignment") newer_than:7d -in:sent
  offers     : ("offer letter" OR "pleased to offer" OR "job offer" OR "total compensation" OR "start date") newer_than:7d -in:sent
  ats        : from:(greenhouse.io OR lever.co OR workday.com OR icims.com OR ashbyhq.com OR smartrecruiters.com OR mail.amazon.jobs) newer_than:7d
  pageSize: 50 for each

  Widen to newer_than:14d if all 5 searches return 0 threads.

STEP OP3  Classify each thread
  FOR each thread:
    company = extract from subject, sender domain, or body
    signal  = interview | oa | offer | rejected | applied (skip applied/confirmation)
    title   = job title if visible

  Fetch full body (get_thread FULL_CONTENT) only when snippet is ambiguous.

STEP OP4  Match to DB and compute diff
  FOR each classified thread:
    1. Exact match: company_name.toLowerCase() in lookup
    2. Fuzzy: check if any word (>3 chars) from email company appears in existing names
    3. No match: will be created

  Skip: DB status is already correct
  Skip: email signal would downgrade status (stage order: applied < oa < interview < offer < accepted)
  Skip: DB status is "rejected" and signal is "applied"

STEP OP5  Apply changes
  Updates → PATCH $BASE/applications/$ID  {status, notes}
  Creates → POST  $BASE/applications       {company_name, job_title, status, date_applied, notes}
  Use email date as date_applied when actual application date is unknown.
  If a company matches multiple DB records, update the most recent one only.
```

---

## Storage API Calls — OfferPlus
...
curl -s -X PATCH "$BASE/applications/$ID" \
  ...
  -d '{"status": "interview", "notes": "Interview invite via Greenhouse — Jun 9"}'
```
...
curl -s -X POST "$BASE/applications" \
  ...
  -d '{
    ...
    "notes": "Interview scheduled Jun 9 — sourced from Gmail"
  }'
```

### Error handling
- 401 → stop immediately; ask user to verify token at Profile → Agent API Keys
- Empty results for all queries → widen to `newer_than:14d` and retry once
- Multiple DB records for same company → update the most recently created one
...
When syncing to both Notion and OfferPlus, print both sections:
...
=== OFFERPLUS ===
Updated: [id] Company — old_status → new_status  (signal: ...)
Added:   [id] Company — status  (signal: ...)
Skipped: Company — already correct / confirmation only / no match
```

Full per-backend format:
